 ...
 * @modifyby Stotskiy Sergiy <serjo@freaksidea.com> 2011
 * @version  5.x $Id$
 * @package  JsHttpRequest
 */
class JsHttpRequest {
    ...
    function reset(){
        $this->ID = 0;
        $this->LOADER = 'unknown';
        $GLOBALS['JsHttpRequest_Active'] = false;
    }
    ...
    function quoteInput($s){
        if ($this->SCRIPT_DECODE_MODE == 'entities') return strtr($s,array('"' => '&quot;', '<' => '&lt;', '>' => '&gt;'));
    ...
    function _obHandler($text){
        ...
        if($this->ID == 0 || $this->LOADER == 'unknown') {
            return $text;
        }
        ...
        if ($text === null)$text = $this->php2js($result);
        if ($this->LOADER != "xml") {
            $text="".($this->LOADER=="form"? 'top && top.$_Request' : '$_Request').".dataReady(" . $text . ")\n". "";
            if ($this->LOADER == "form")
